...
void fancyPrintTree(MemoryBlock *root, int level)
{
    if (root == NULL)
        return;

    printf("%s", root->process_pid != -1 ? COLOR_GREEN : COLOR_RED);

    for (int i = 0; i < level; i++)
        printf("|  ");

    printf("%s Process ID: %d\n",
           root->process_pid == -1 ? "Free" : "Allocated", root->process_pid);

    printf(COLOR_RESET);

    fancyPrintTree(root->left_child, level + 1);
    fancyPrintTree(root->right_child, level + 1);
}
MemoryBlock *traverse_MemorySegment(MemoryBlock *root, int needed_memory)
{
    if (root == NULL || root->size < needed_memory)
        return NULL;
...
bool allocate_memory(int proc_pid, int memory_size)
{
    const int memory_needed = highestPowerOf2(memory_size);
    ...
    if (new_block != NULL)
    {
        new_block->id_from_file = proc_pid;
        // mp[new_block->process->pid]=new_block;
        new_block->allocated = memory_size;
        ...
        fancyPrintTree(Memory_Segment, 0);
        return true;
    }
...
{
    MemoryBlock *found = try_get_Process_Memory_Segment(Memory_Segment, pid_from_file);
    if (found)
    {
        found->process_pid=pid;
        printf("SAROOO22AAAAAAAAAA\n\n");
    }
}
...
MemoryBlock *try_get_Process_Memory_Segment(MemoryBlock *root, int id_from_file)
{
    ...
    if (root->id_from_file == id_from_file)
        return root;

    MemoryBlock *left_result = try_get_Process_Memory_Segment(root->left_child, id_from_file);
    if (left_result)
        return left_result;

    return try_get_Process_Memory_Segment(root->right_child, id_from_file);
}

bool deallocate_memory(pid_t pid)
{
    MemoryBlock *to_delete = get_Process_Memory_Segment(Memory_Segment, pid);
